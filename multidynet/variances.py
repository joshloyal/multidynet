...
def update_sigma_sq(Y, X, X_sigma, X_cross_cov, c, d):
    ...
    c_sigma_sq = (c +
        (n_nodes - 1) * (n_time_steps - 1) * n_features)
    d_sigma_sq = d
    for t in range(1, n_time_steps):
        ...
        d_sigma_sq += np.trace(
            X_sigma[t-1, :-1], axis1=1, axis2=2).sum()
        d_sigma_sq += (X[t-1, :-1] ** 2).sum()

        d_sigma_sq -= 2 * np.trace(
            X_cross_cov[t-1, :-1], axis1=1, axis2=2).sum()
        d_sigma_sq -= 2 * (X[t-1, :-1] * X[t, :-1]).sum()
