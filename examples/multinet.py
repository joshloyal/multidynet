...
from multidynet.metrics import score_latent_space, score_social_trajectories


seed = 0 
n_features = 2
Y, X, lmbda, delta, probas, dists, z = correlated_dynamic_multilayer_network(
    n_nodes=100, n_layers=5, n_time_steps=10, n_features=n_features,
    center=0.75, tau=[0.5, 0.75], include_delta=True,
    rho=0., rho_t=0.4, sigma=0.05,
    random_state=seed)
#seed = 0
#n_features = 2
...
#Y_train, test_indices = train_test_split(Y, test_size=0.1, random_state=23)
#
...
#                tol=1e-2, init_type='svt', n_init=10,
